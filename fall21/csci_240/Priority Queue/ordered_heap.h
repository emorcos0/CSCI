...
#ifndef ORDERED_HEAP_H
#define ORDERED_HEAP_H
...
template <typename T>
class OrderedHeap : Heap<T>
{
public:
    OrderedHeap(std::function<bool(T, T)> comparator) : OrderedHeap(DEFAULT_SIZE,comparator)
    {
        
    }

    OrderedHeap(const T array[], size_t size, std::function<bool(T, T)> comparator)
    {
    ...
    OrderedHeap(size_t initialSize, std::function<bool(T, T)> comparator) : store{new T[initialSize + 1]}, currentSize{initialSize + 1}
    {
        this->comparator = comparator;
    }
